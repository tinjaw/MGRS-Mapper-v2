...
// Convert an MGRS string to Lat/Lon
...
function latLngFromMGRS(a) {
  ...
  const p = o + (0.0025188266133249035 * Math.sin(2.0 * o)) + (0.0000037009491206268 * Math.sin(4.0 * o)) + (0.0000000074477705265 * Math.sin(6.0 * o)) + (0.0000000000170359940 * Math.sin(8.0 * o));
  const q = Math.tan(p);
  const r = q * q;
  const s = r * r;
  const t = Math.cos(p);
  ...
  const x = 1.0 / (w * t); w *= v;
  const y = q / (2.0 * w); w *= v;
  const z = 1.0 / (6.0 * w * t); w *= v;
  const aa = q / (24.0 * w); w *= v;
  const ab = 1.0 / (120.0 * w * t); w *= v;
  const ac = q / (720.0 * w); w *= v;
  const ad = 1.0 / (5040.0 * w * t); w *= v;
...
export {
  L, map, generateGZDGrids, generate100kGrids, generate1000meterGrids, LLtoUTM, UTMtoMGRS, latLngFromMGRS, natGeoMap, OSMMap, tonerLiteMap, worldSatelliteMap, worldTopoMap,
};
